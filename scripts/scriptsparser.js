document.getElementById('searchInput').addEventListener('input', function() {
    ...
    document.getElementById('parseButton').addEventListener('click', function() {
        ...
        const fields = [
          ...
          "Please provide the testimonials you'd like to display",
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // Check if the line starts with CX or Sales or is a date/time
            if (cxSalesPattern.test(trimmedLine) || dateTimePattern.test(trimmedLine)) {
                if (currentField) {
                    // Store any remaining multi-line detail before resetting
                    results[currentField] = multiLineDetail.trim();
                    currentField = null; // Reset currentField
                    multiLineDetail = ''; // Reset multiLineDetail
                }
                continue; // Skip this line
            }

            // Check if the line matches any of the defined fields
            if (fields.includes(trimmedLine)) {
                if (currentField) {
                    // Store any existing multi-line detail before starting a new field
                    results[currentField] = multiLineDetail.trim();
                }
                currentField = trimmedLine; // Start capturing details for this field
                multiLineDetail = ''; // Reset multiLineDetail for new field
                continue; // Move to the next line
            }

            // If we have a currentField, capture the detail (multi-line)
            if (currentField) {
                multiLineDetail += trimmedLine + '\n'; // Append the line to multiLineDetail
            }
        ...
        // Store any remaining detail after the last field
        if (currentField) {
            results[currentField] = multiLineDetail.trim();
        }
